...
❯ 加一条规则：结论必须标明来源
❯ 我需要一个专门看财报的专家
❯ 把 ollama 上的 kimi-k3 加进来
```

编排者有三个工具：`create_rule`、`create_agent`、`configure_model`。它们要
`configure` 权限，而**默认只有入口 agent 有** —— 专家看不到它们
（工具不出现在它看到的定义里，那是边界层）。

判定仍然全部由运行时做：工具名是否真实、字段名是否 snake_case、
是否只剩提醒、权限是否可授予、密钥有没有混进配置。模型判不了的那些一个都没放松。

`configure_model` 会**保留配置里的注释**（外科式插入，不是重写整个文件），
而且写盘前先自检一遍：解析得回来、数组只多一项、新项的 key 对得上。
宁可不写，也不要留下一份起不来的配置。

#### 它能提权，这件事说清楚

编排者自己没有 `write` / `execute`。但它可以**造一个带那些权限的专家、
然后委派给它** —— 那等于给自己发了那些权限。

这个模型是**刻意接受**的（不逐次批准），代价用可见性补：

- 每次写入的完整内容都回到对话里
- 全量进 `logs/`，事后能查是谁在什么时候改了什么
- 每次都给出回退命令
- 授了 `execute` / `write` / `network` 时**单独点出来**，不混在一串 id 里

不加闸门不等于不说话。要收紧的话，从 `agents/orchestrator.md` 的
`permissions` 里去掉 `configure` —— 那三个工具就立刻不可见。
