...
export function askUserTool(store: RunStore, conversations: ConversationAppender): ToolDefinition {
  return {
    ...
    description:
      /**
       * ── 「什么时候不要问」和「什么时候问」一样重要 ────────────
       *
       * 第一版只写了「需求含糊时先问，别猜」。实测 gemma4 在**任务已经做完之后**
       * 调它来确认「还有没有别的需要」—— 那在客服语境里是礼貌，在这里是浪费
       * 一整轮，而且把 run 挂在 waiting_user、占着会话锁：你不回话它就不结束。
       *
       * 写了「先问」而没写「做完了就别问」，等于只给了半个判据。
       */
      '在**动手之前**问一个必须先弄清的问题，然后这一轮结束、等用户回答。\n' +
      '\n' +
      '什么时候问：**需求含糊到你只能靠猜** —— 猜错要等整条委派链跑完才看得出来。\n' +
      '  例：「帮我写份报告」→ 什么主题？给谁看？\n' +
      '\n' +
      '**什么时候不要问（更要紧）：**\n' +
      '- **活已经干完了就不要问。** 做完了就 submit_result 收尾。\n' +
      '  「还有别的需要吗」不是提问，是把一个已完成的任务挂起来等人回话。\n' +
      '- 不要用它汇报进度或确认你刚做的事对不对 —— 结果本身就是汇报。\n' +
      '- 能自己合理决定的不要问。问一次的代价是使用者被打断一次。\n' +
      '\n' +
      '一次只问一件事。',
    parameters: {
      type: 'object',
