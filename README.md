...
要有 worker 在跑才会执行：nucleus serve
接下来：2026-08-03 08:30 · 2026-08-04 08:30 · 2026-08-05 08:30
```

不需要**单独的调度器**：`worker.tick()` 里就地触发，和 reconciler 同一个位置 ——
「定时」的本质就是到点往队列塞一个 run。但 worker 得有人在跑，那就是
`nucleus serve`（空闲的 `nucleus chat` **不**推进任何东西 —— 只有你打一句话
才 drain 一次）。
...
- **定时 run 里 `ask_user` 被直接拒绝。** 信封里一直写着「没有人在线」，
  而那只是一句话 —— 实测 gemma4 照样调了 `ask_user`，run 停在 `waiting_user`
  等一个永远不会来的回答，**而重入保护让后面每一次触发都被跳过**：
  一次提问把整条计划永久锁死。现在按 `scheduleId` 挡死，并告诉它把要问的
  写进 `open_questions`。又一次「提醒代替机制」，而这次机制是有的。
...
- 真 Postgres 路径**已验证**（14.17：迁移、doctor、端到端委派链、三进程并发迁移、
  1110 passed 0 skipped）。托管数据库与 14 以外的版本仍未验。
